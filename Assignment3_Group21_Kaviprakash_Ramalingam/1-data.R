...
## [your code here]
dfsubset <- filter(df, Grad.Rate >= 80)
dfsubset
arrange(dfsubset, desc(Grad.Rate))
arrange(dfsubset, desc(Grad.Rate)) %>% select(X,Grad.Rate)
dfsubset
...
## [your code here]
dfexpense <- select(df, Private, Outstate)
dfexpense <- group_by(dfexpense, Private)
dfexpense <- summarise(dfexpense, max = max(Outstate), min = min(Outstate))
dfexpense

colsub <- select(college, Private, Outstate)
colsub <- group_by(colsub, Private)
colsub <- summarise(colsub, max = max(Outstate), min = min(Outstate))
colsub
